...
const PacienteScreen = ({ navigation }: Props) => {
  ...
  const fetchPacientes = async () => {
    ...
    const response = await fetch("http://127.0.0.1:8000/paciente/api/");
  ...
  const handleDelete = async (id: number) => {
    await fetch(`http://127.0.0.1:8000/paciente/api/${id}/`, {
      method: "DELETE",
    });
  ...
  const renderItem = ({ item }: { item: Paciente }) => (
    <View style={styles.card}>
      ...
      <Text style={styles.description}>Nascimento: {item.data_nascimento}</Text>

      <Text style={styles.description}>Peso: {item.peso} kg</Text>

      <Text style={styles.description}>Altura: {item.altura} m</Text>
