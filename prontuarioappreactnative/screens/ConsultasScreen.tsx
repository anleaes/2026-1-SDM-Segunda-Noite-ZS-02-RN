...
import { Consulta, DrawerParamList } from "../navigation/DrawerNavigator";

type Props = DrawerScreenProps<DrawerParamList, "Consultas">;

const ConsultasScreen = ({ navigation }: Props) => {
  ...
  const handleDelete = async (id: number) => {
    await fetch(`http://127.0.0.1:8000/consulta/api/${id}/`, {
      method: "DELETE",
    });
  ...
  const renderItem = ({ item }: { item: Consulta }) => (
    <View style={styles.card}>
      <Text style={styles.name}>Consulta N° {item.id}</Text>

      <Text style={styles.description}>Data: {item.data_agendada}</Text>
      ...
      <View style={styles.row}>
        <TouchableOpacity
          ...
          onPress={() =>
            navigation.navigate("EditConsulta", {
              consulta: item,
            })
          }
        >
